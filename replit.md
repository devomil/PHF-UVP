...
## Recent Changes
- 2026-02-19: Brand Settings persistence and pipeline integration. Added brandSettings database table (brand_name, tagline, website, colors, logo_url, guidelines). Created /api/brand-settings endpoints (GET/PUT for settings, POST/DELETE for logo upload). Rewrote brand-settings.tsx with full state management, color pickers, drag-drop logo upload, and auto-save feedback. Brand context injected into project creation (brand field auto-populated), script parser (dynamic brand name replaces hardcoded Pine Hill Farm), and video prompt optimizer. Created brand-settings-service.ts with caching for efficient brand context lookups during asset generation.
- 2026-02-19: Fixed reference image pipeline. Project-level reference images (uploaded in Step 2: Configure & Generate Assets) are now persisted in assets JSON and distributed to matching scene types (solution, CTA, product, feature, benefit, hook) before generation. Reference images set scene.brandAssetUrl to trigger I2V generation. URLs resolved to full public URLs for external AI providers. saveProjectToDb embeds referenceImages in assets for persistence across all code paths.
- 2026-02-19: Configurable T2V vs I2V pipeline. Added `videoGenerationMode` field to schema/types ('direct-t2v' | 'image-first-i2v' | 'auto'). Default behavior changed from image-first-I2V to direct T2V for video-mode projects, skipping redundant intermediate image generation. Users can choose "Direct T2V (faster)" or "Image then Video (more control)" in project creation UI. I2V only used when user explicitly provides reference images. Scene editor shows T2V/I2V mode indicator. Enhanced scene editor: fixed reference image upload wiring, library browser uses FolderOpen icon with inline asset grid, separate Image/Video regeneration buttons.
