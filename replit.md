...
- **Social Login (Task #163)**: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, optional `GOOGLE_CALLBACK_URL` (default `${APP_URL}/api/auth/google/callback`); `FACEBOOK_APP_ID`, `FACEBOOK_APP_SECRET`, optional `FACEBOOK_CALLBACK_URL`. Each provider is registered conditionally — missing env vars cause the strategy + buttons to be omitted gracefully (no crash). `GET /api/auth/providers` reports which are configured. OAuth signups create users via `linkOrCreateOAuthUser` in `server/auth.ts` which (1) matches existing `oauth_accounts` row, else (2) links by verified email to existing `users` row, else (3) creates a new user (role `employee` unless email is in `ADMIN_EMAILS`), provisions free trial via `createInitialTrialForNewUser`, and fires signup + welcome notifications. Linkage stored in `oauth_accounts` table (unique `(provider, provider_account_id)`).
