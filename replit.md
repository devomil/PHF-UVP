...
-   **Visual Art Direction Presets**: 8 visual art style presets (3D Illustration, Cinematic Realism, 2D Line Art, Collage, Claymation, Neon Futuristic, Watercolor, Minimalist Flat) defined in `shared/config/visual-art-presets.ts`. Selected during project creation via horizontal scrollable card UI with gradient thumbnails. Each preset injects `imagePromptPrefix`/`imagePromptSuffix` into all scene image/video prompts, influences Claude visual direction generation, adjusts provider selection preferences, and sets generation strategy (I2V/T2V/auto). Prompt sanitizer whitelists art preset keywords; prompt optimizer preserves style tokens. Stored as `artPresetId` in project data.
-   **Smart Text Label Overlays**: Claude-powered text label extraction (`server/services/text-label-extractor.ts`) analyzes scene narrations to identify key terms, statistics, and concepts that should appear as on-screen text. Labels are rendered by `TextLabelOverlay` Remotion component (`remotion/components/overlays/TextLabelOverlay.tsx`) with 8 visual treatments (badge, floating-tag, holographic-panel, handwritten, neon-glow, minimal, pill, underline). Art-preset-aware treatment hints ensure labels match the video's visual style. This produces crisp, correctly spelled English text overlays instead of AI-garbled text.
