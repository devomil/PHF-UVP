...
\`\`\`json
{"suggestedPrompt": "your visual direction here", "suggestedArtStyle": {"id": "preset-id", "name": "Preset Name", "rationale": "one sentence explaining why this style fits the scene"}, "suggestedProvider": "provider-id-here", "suggestedProviderRationale": "1-2 sentence explanation of why this provider fits the scene", "suggestedNegativePrompt": "negative terms here", "suggestedCfgScale": 0.85}
\`\`\`

- **suggestedPrompt** (required when you have a prompt): The visual direction text
- **suggestedArtStyle** (required when you have a prompt): The art style preset that best matches this prompt. Pick from the available presets listed above. Consider the scene content, narration, and brand — e.g., clinical/health content pairs well with "scientific-medical", product showcases with "cinematic-realism", playful/fun brands with "3d-illustration" or "claymation", etc. If the user already has an art style selected, recommend keeping it unless a different one would be clearly better. Always include a \`rationale\` field inside the object: one concise sentence explaining WHY this style fits — e.g., "Chosen for its cinematic depth and brand color match" or "Ideal for this health scene's clean, clinical aesthetic." Ground it in the scene content, narration mood, or brand identity.
