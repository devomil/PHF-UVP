import { VIDEO_PROVIDERS, IMAGE_PROVIDERS } from './provider-config';
...
export const VIDEO_PROVIDER_CATALOG: ProviderCatalogEntry[] = [
  ...
  },
  {
    ...
    family: 'Runway',
    description: 'Best-in-class frame-based video editing. Edit one reference frame and Aleph 2.0 modifies the entire clip to match — perfect for re-styling, lighting correction, and cinematic refinement. Direct API.',
    capabilities: ['V2V'],
    maxDuration: 10,
    costTier: 'premium',
    type: 'video',
  ...
  },
  {
    ...
    family: 'Runway',
    description: 'New Runway-native creative model with expressive, stylized motion. Excellent for creative, playful, and character-driven content. Direct API.',
    capabilities: ['T2V', 'I2V'],
    maxDuration: 10,
    costTier: 'premium',
    type: 'video',
  ...
  },
  {
    ...
    family: 'Seedance',
    description: 'Seedance 2 with expanded content permissions — adds full-face support and reduced content filtering. Same 1080p quality and 15s max duration as Seedance 2.',
    capabilities: ['T2V', 'I2V', 'Audio'],
    maxDuration: 15,
    costTier: 'standard',
    type: 'video',
...
export const IMAGE_PROVIDER_CATALOG: ProviderCatalogEntry[] = [
  ...
  },
  {
    ...
    name: 'GPT-Image-2',
    family: 'OpenAI',
    description: 'OpenAI\'s GPT-Image-2 — the latest generation of OpenAI image models. Superior prompt adherence, high-detail output, and reliable quality for both realistic and creative content.',
    capabilities: ['T2I'],
    maxDuration: 0,
    costTier: 'premium',
    type: 'image',
  ...
  },
  {
    ...
    family: 'ByteDance',
    description: 'ByteDance Seedream 5 Lite via PiAPI. High-quality aesthetic image generation with strong composition and lighting — great for lifestyle and creative content.',
    capabilities: ['T2I'],
    maxDuration: 0,
    costTier: 'standard',
    type: 'image',
...
// Derives multi-image capability from VIDEO_PROVIDERS and IMAGE_PROVIDERS in
// provider-config.ts (the authoritative source for provider capabilities).
// Returns true when the provider has a multiImageSupport object configured
// (i.e. supports @imageN syntax).
...
export function providerSupportsMultiImage(providerId: string | undefined | null): boolean {
  if (!providerId) return false;
  return !!(VIDEO_PROVIDERS[providerId]?.multiImageSupport || IMAGE_PROVIDERS[providerId]?.multiImageSupport);
}
